            ...
            if (res.ok) {
                ...
                allGuests.forEach((g) => {
                    formData[g.id] = {
                        ...
                        welcome_drinks_accepted_status:
                            g.welcome_drinks_accepted_status ?? null,
        ...
        try {
            const payload = {
                guests: guestsToSubmit.map((g) => {
                    ...
                    const welcomeStatus =
                        fd.welcome_drinks_accepted_status ?? null;
                    return {
                        guest_id: g.id,
            ...
            if (res.ok) {
                const groupRes = await fetch(
                    `${API_URL}/api/guests/${guestGroup.primary.id}/group`
                );
                if (groupRes.ok) {
                    ...
                    refreshedGuests.forEach((g) => {
                        formData[g.id] = {
                            wedding_accepted: g.wedding_accepted ?? 0,
                            welcome_drinks_accepted:
                                g.welcome_drinks_accepted ?? 0,
                            wedding_accepted_status:
                                g.wedding_accepted_status ?? null,
                            welcome_drinks_accepted_status:
                                g.welcome_drinks_accepted_status ?? null,
                            allergies: (g.allergies || []).join(", "),
                        };
            ...
            >
                ...
                {modalStep === "group" &&
                    ...
                    (() => {
                        const allGuests = [
                            guestGroup.primary,
                            ...(guestGroup.accompanied || []),
                        ];
                        const incompleteGuests = allGuests.filter(
                            (g) => !isRsvpComplete(g)
                        );
                                            ...
                                            guestsToEdit.map((guest) => {
                                                const fieldPrefix =
                                                    rsvpStep === 0
                                                        ...
                                                        : "welcome_drinks";
                                                const status =
                                                    rsvpFormData[guest.id]?.[
                                                        `${fieldPrefix}_accepted_status`
                                                    ] ?? null;
                                                const attendingSelected =
